...
/**
 *  @route GET /farmer/farm/:farmId/reservation
 *  @desc Read Reservation by farmId
 *  @access Private
 */
const getFarmerReservationByFarmId = async (req: Request, res: Response) => {
    const { farmId } = req.params;

    try {
        const data = await FarmService.getFarmerReservationByFarmId(farmId);
